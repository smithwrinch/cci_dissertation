...
class LucidSonicDream:
  def __init__(self,
    ...
    # ADD WRONG DIR TO GET EXCEPTION
    self.Gs = tf.keras.models.load_model(gan_dir)
  ...
  def generate_frames(self, txt_dir):
    ...
    for i in range(num_frame_batches):
        ...
        f = open(txt_dir, "w")
        f.write(str(i) + " frames completed of " + str(num_frame_batches) + " frames")
        f.close()
        ...
        image_batch = self.Gs(noise_batch);
  ...
  def hallucinate(self,
    ...
    print('\n\nHallucinating... \n')
    f = open(txt_dir, "w")
    f.write("Hallucinating")
    f.close();
    self.generate_frames(txt_dir)
    ...
    f = open(txt_dir, "w")
    f.write("Creating temporary audio file....")
    f.close()
    ...
    f = open(txt_dir, "w")
    f.write("Writing video file....")
    f.close()
    video.write_videofile(file_name,audio_codec='aac', logger=None, verbose=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--song_dir", required=True)
    parser.add_argument("--txt_dir", required=True)
    parser.add_argument("--gan_dir", required=True)
    parser.add_argument("--latent_dim", type=int, required=True)
    parser.add_argument("--speed_fpm", type=int, default=12, required=False)
    parser.add_argument("--pulse_react", type=float, default=0.5, required=False)
    parser.add_argument("--motion_react", type=float, default=0.5, required=False)
    parser.add_argument("--pulse_percussive", default="1", required=False)
    parser.add_argument("--pulse_harmonic",  default="0", required=False)
    parser.add_argument("--motion_percussive",  default="0", required=False)
    parser.add_argument("--motion_harmonic", default="1", required=False)
    parser.add_argument("--motion_randomness", type=float, default=0.5, required=False)
    parser.add_argument("--save_frames", default="0", required=False)
    parser.add_argument("--fps", type=int, default=43, required=False)
    ...
    SPEED_FPM = args.speed_fpm
    PULSE_REACT = args.pulse_react
    MOTION_REACT = args.motion_react
    PULSE_PERCUSSIVE = str2Bool(args.pulse_percussive)
    PULSE_HARMONIC = str2Bool(args.pulse_harmonic)
    MOTION_PERCUSSIVE = str2Bool(args.motion_percussive)
    MOTION_HARMONIC = str2Bool(args.motion_harmonic)
    MOTION_RANDOMNESS = str2Bool(args.motion_randomness)
    SAVE_FRAMES = str2Bool(args.save_frames)

    print(PULSE_HARMONIC)
    print(SAVE_FRAMES == True)
    FPS = args.fps
    f = open(TXT_DIR, "w")
    f.write("Starting setup (this may take a while..)")
    f.close()
    L = LucidSonicDream(song = SONG_DIR, gan_dir = GAN_DIR, input_shape=LATENT_DIM)

    L.hallucinate(file_name = FNAME, speed_fpm = SPEED_FPM, pulse_react=PULSE_REACT,
    motion_react=MOTION_REACT, pulse_percussive= PULSE_PERCUSSIVE, pulse_harmonic=PULSE_HARMONIC,
    motion_percussive = MOTION_PERCUSSIVE, motion_harmonic=MOTION_HARMONIC, motion_randomness=MOTION_RANDOMNESS,
    save_frames = SAVE_FRAMES, fps = FPS, txt_dir = TXT_DIR)


    f = open(TXT_DIR, "w")
    f.write("Finished!")
    f.close()
    # /home/hans/uni/diss/main/IML_TOOL/bin/data/lucidsonicdreams/song.mp3
    # /home/hans/uni/diss/main/IML_TOOL/bin/data/lucidsonicdreams/song.mp3

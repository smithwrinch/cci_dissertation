...
//--------------------------------------------------------------
void InputPix2PixScene::setup() {
  setID(SCENE_TYPE::INTERACTPIX2PIX);
	ofSetFrameRate(60);
	...
	if(!model.load("default_models/edges2shoes")) {
  ...
  backButton->onButtonEvent(this, &InputPix2PixScene::onButtonEvent);

}

void InputPix2PixScene::startThread(){
  autoRun = true;
}

void InputPix2PixScene::stopThread(){
    autoRun = false;
}

void InputPix2PixScene::update() {
...
//--------------------------------------------------------------
void InputPix2PixScene::keyPressed(int key) {
	switch(key) {
...
//--------------------------------------------------------------
void InputPix2PixScene::mouseDragged( int x, int y, int button) {
	switch(drawMode) {
		case 0: // draw
			fbo.begin();
			ofSetColor(drawColor);
...
//--------------------------------------------------------------
void InputPix2PixScene::mousePressed( int x, int y, int button) {
	mousePressPos = glm::vec2(x, y);
	mouseDragged(x, y, button);
}

//--------------------------------------------------------------
void InputPix2PixScene::mouseReleased(int x, int y, int button) {
	switch(drawMode) {
		case 0: // draw
			break;
		case 1: // draw boxes
...
//--------------------------------------------------------------
void InputPix2PixScene::dragEvent(ofDragInfo dragInfo) {
...
void InputPix2PixScene::onButtonEvent(ofxDatGuiButtonEvent e){
...
void InputPix2PixScene::setupDrawingTool(std::string model_dir) {
...
template <typename T>
bool InputPix2PixScene::drawImage(const T& img, string label) {
	if(img.isAllocated()) {
		ofSetColor(255);
		ofFill();

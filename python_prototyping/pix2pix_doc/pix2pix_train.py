...
def fit(train_ds, test_ds, steps, g_opt, d_opt, gen_loss_, disc_loss_, LAMBDA, checkpoint, gen, disc):
    ...
    for step, (input_image, target) in train_ds.repeat().take(steps).enumerate():
        if (step) % 1000 == 0:
              ...
              generate_images(gen, example_input, example_target, step)
              print(f"Step: {step//1000}k")

        train_step(input_image, target, step, g_opt, d_opt, gen_loss_, disc_loss_, LAMBDA, gen, disc)
...
def generate_images(model, test_input, tar, step):
    ...
    for i in range(3):
        ...
        plt.axis('off')
    plt.savefig("imgs_new/"+str(step)+".png")
...
if __name__ == '__main__':
    ...
    fit(train_dataset, test_dataset, MAX_EPOCHS, generator_optimizer, discriminator_optimizer, GEN_LOSS, DISC_LOSS, LAMBDA, checkpoint, generator, discriminator)
